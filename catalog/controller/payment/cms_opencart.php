...
Module Version. Cms-1.3  			Module release: August 16/2017
**************************************************************************************
 */
error_reporting(E_ALL);
class ControllerPaymentCmsOpencart extends Controller {
	protected function index() {
		$this->language->load('payment/cms_opencart');

		$this->data['button_confirm'] = $this->language->get('button_confirm');
        $this->data['cms_opencart_mode'] 	= $this->config->get('cms_opencart_mode');
        $mode = $this->data['cms_opencart_mode'];
        $this->data['url'] 		        = $this->config->get('cms_opencart_url');

        if($mode == '1'){
           
            $this->data['action'] = $this->config->get('cms_opencart_live_url');//live mode url
          
        }
        else{
           
			  $this->data['action'] = $this->config->get('cms_opencart_test_url');//live mode url
		...
		foreach ($products as $product) {
            ...
            $this->data['title'] 		    = $this->config->get('cms_opencart_title');
            $this->data['description'] 		= $this->config->get('cms_opencart_description');
			$this->data['Merchant_Id'] 		= $this->config->get('cms_opencart_merchant_id');
		    $this->data['Order_Id'] 		= $this->session->data['order_id'];
			$this->data['Amount'] 			= $Amount;
			$this->data['currency_code'] 	= $currency;
			$this->data['totype']       	= $this->config->get('cms_opencart_partner_name');
			$this->data['partenerid']       	= $this->config->get('cms_opencart_partner_id');
			$this->data['ipaddr']       	= $this->config->get('cms_opencart_ipaddr');
			$this->data['cms_opencart_mode'] 	= $this->config->get('cms_opencart_mode');

				$Merchant_Id= $this->config->get('cms_opencart_merchant_id');
				$totype= $this->config->get('cms_opencart_partner_name');
				$partenerid= $this->config->get('cms_opencart_partner_id');
				$ipaddr= $this->config->get('cms_opencart_ipaddr');
				$Order_Id= $this->session->data['order_id'];
				$Amount= $this->data['Amount'];

            $Url= $this->url->link('payment/cms_opencart/callback');
				$WorkingKey= $this->config->get('cms_opencart_working_key');
						$pattern 		= '#http://www.#';
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/cms_opencart.tpl')) {
				$this->template 	= $this->config->get('config_template') . '/template/payment/cms_opencart.tpl';
			} else {
				$this->template 	= 'default/template/payment/cms_opencart.tpl';
			}
			$this->render();
		}
	}
      ...
      $this->DBG_error("\n WorkingKey " . date("Y:m:D h:i:s") . "\n" . $WorkingKey);
		...
		if($order_info)
		{
			...
			{	
				...
				$payment_status  = true;
				$order_status_id = $this->config->get('cms_opencart_completed_status_id');
				...
				$payment_confirmation_mail =  $this->config->get('cms_opencart_payment_confirmation_mail');
				if($payment_confirmation_mail)
				{					
					$subject  = 'Direcpay Payment Status';
					$text     = "Dear ".$order_info['firstname']." ".$order_info['lastname']. "\n\n";
					$text    .= "We have received your order, Thanks for your Cms opencart payment.The transaction was successful.Your payment is authorized.". "\n";
					...
					$mail->send();
					mail($order_info['email'],'Cms opencart Payment Status','Your payment is authorized.');
				}
			
			}
			else if($Checksum=="true" && $AuthDesc=="C")
			{	
				//echo $AuthDesc;
				//die;
				$payment_status_message = $this->language->get('canceled_comment');
				$payment_status = false;
				$order_status_id = $this->config->get('cms_opencart_canceled_status_id');
				if (!$order_info['order_status_id']) {
					$this->model_checkout_order->confirm($Order_Id, $order_status_id, $payment_status_message);
				} else {
					$this->model_checkout_order->update($Order_Id, $order_status_id, $payment_status_message, FALSE);
			...
			else if($Checksum=="true" && $AuthDesc=="N")
			{
				...
				$payment_status = false;
				$order_status_id = $this->config->get('cms_opencart_failed_status_id');
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/cms_opencart_response.tpl')) {

					$this->template = $this->config->get('config_template') . '/template/payment/cms_opencart_response.tpl';

				} else {

					$this->template = 'default/template/payment/cms_opencart_response.tpl';
